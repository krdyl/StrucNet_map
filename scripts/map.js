$(window).on('load', function() {
  ...
  function onMapDataLoad(options, points, polylines) {
    ...
    document.title = 'Cavelab TLS data';
    addBaseMap();
...
//  }
  ...
  /**
   * Moves the terminator on the map
   */
  //var t = L.terminator().addTo(map);
   ...
   */
   ...
   $.ajax({
       url:'./csv/umd-global-tls-metadata-config-options.csv',
       type:'HEAD',
       error: function() {
          ...
          $.getJSON(
            apiUrl + configspreadsheetId + '?key=' + googleApiKey
              ...
              $.when(
                ...
                $.getJSON(apiUrl + configspreadsheetId + '/values/Polylines?key=' + googleApiKey)
                ...
                var fetchPolygonsSheet = function(polygonSheets) {
                  ...
                  if (polygonSheets.length === 0) {
                    ...
                    $.getJSON(apiUrl + configspreadsheetId + '/values/' + polygonSheets.shift() + '?key=' + googleApiKey, function(data) {
       ...
       */
       success: function() {
        ...
        $.when(
          $.get('./csv/umd-global-tls-metadata-config-options.csv'),
          $.get('./csv/cavelab-metadata.csv'),
          $.get('./csv/umd-global-tls-metadata-config-polylines.csv')
        ).done(function(options, points, polylines) {

          function loadPolygonCsv(n) {

            $.get('./csv/umd-global-tls-metadata-config-polygons' + (n === 0 ? '' : n) + '.csv', function(data) {
              createPolygonSettings( parse([data]) )
              loadPolygonCsv(n+1)
            }).fail(function() {
